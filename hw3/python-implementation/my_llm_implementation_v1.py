...
class Embedding(DummyEmbedding):
    def forward(self, idx):
        ...
        position_idx = torch.arange(0, T, dtype=torch.long, device=idx.device)
        position_idx = position_idx.unsqueeze(0).expand(B, T)
        position_emb = self.position_embeddings(position_idx)
        embeddings = token_embeddings + position_emb                                               #
        #############################################################################
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
class GenericTransformer(DummyTransformer):
    ...
    def forward(self, idx, targets=None, hidden_cache=None, return_hidden=False):
        ...
        if hidden_cache is not None:
            x = torch.cat([hidden_cache, token_embeddings], dim=1)
        else:
            x = token_embeddings
...
class Encoder(GenericTransformer):
    ...
    def get_attention_mask(self, num_tokens):
        ...
        valid_tokens = torch.arange(max_tokens, device=num_tokens.device) < num_tokens[:, None]
        attention_mask = valid_tokens[:, None, :].expand(B, max_tokens, max_tokens)
        # mask = (torch.arange(max_tokens, device=num_tokens.device).unsqueeze(0) < num_tokens.unsqueeze(1)).to(torch.int)
        # attention_mask = mask.unsqueeze(1).expand(B, max_tokens, max_tokens) 
        # mask_1d = torch.arange(max_tokens, device=num_tokens.device).unsqueeze(0) < num_tokens.unsqueeze(-1)
        # attention_mask = (mask_1d.unsqueeze(1) & mask_1d.unsqueeze(2)).int()       
        ##############################################################################
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
class Decoder(Encoder):
    ...
    def get_attention_mask(self, num_tokens):
        ...
        # attention_mask = full_attention_mask * torch.tril(torch.ones_like(full_attention_mask))
        ##############################################################################
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def prefix_generate(model, prefix, max_new_tokens, temperature=1.0):
    ...
    # Hint: Pay close attention to how EncoderDecoder’s forward function uses    #
    # the ‘targets’ parameter                                                    #
    ##############################################################################
    # # Create an empty tensor on the same device as prefix
    # B = prefix.shape[0]
    # idx = torch.zeros((B, 0), dtype=torch.long, device=prefix.device)
    
    # # Generate tokens one by one
    # for _ in range(max_new_tokens):
    #     # Pass both prefix and current tokens to the model
    #     logits, _ = model(prefix, targets=idx)
